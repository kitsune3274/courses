...
def get_pids(port: int) -> List[int]:
    ...
    pids: List[int] = []
    ports = subprocess.Popen(['lsof', '-i', ':5000'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    output, error = ports.communicate()
    # разбить на "линии", пройтись по линиям и получить значения только по столбцу PID, добавив их в pids
...
def free_port(port: int) -> None:
    ...
    if pids:
        for item in pids:
            subprocess.run(['kill', '-9', str(item)])
    return
