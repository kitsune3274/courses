...
class BlockErrors:
    ...
    # если exc_type является подклассов классов в errors или если exc_type находится в errors,
    # то возвращаем True (игнорируем ошибку)
        if issubclass(exc_type, tuple(self.errors)) or exc_type in self.errors:
            return True

if __name__ == "__main__":
    error_type = {FileNotFoundError}
    with BlockErrors(error_type):
        f = open('abc.txt', 'r')
    print('OK')